...
|------|-------------|----------|
| `BarsHistoryDb.cs` | `daemon/Engine/` | SQLite хранилище исторических баров (отдельная БД) |
| `BacktestEngine.cs` | `daemon/Engine/` | Replay loop: загрузка баров → побарный прогон через StrategyProcess → исполнение → сбор результатов |
| `BacktestExecutor.cs` | `daemon/Engine/` | Виртуальное исполнение сделок на исторических барах (SL/TP проверка, P&L, margin sim) |
| `cost_model.json` | `daemon/` | Per-symbol cost model: spread + slippage по asset class (spread_open_widened для справки) |

### Изменяемые файлы

| Файл | Что добавляется |
|------|----------------|
| `DashboardServer.cs` | ~10 новых WS команд для вкладки Tester |
| `index.html` | Вкладка Tester (UI) с dual-view метриками ($ и R) |
...
public class BacktestConfig
{
    ...
    public double CommissionPerLot { get; set; } // комиссия round-trip
    // Cost model
...
1. ПОДГОТОВКА
   ...
   d. Построить timeline: отсортированный список уникальных timestamps
      (каждый timestamp = один "тик" тестера, все символы на этом баре)
...
2. ЗАПУСК СТРАТЕГИИ
   ...
   c. Первый TICK с warmup барами → suppress ENTER (warmup gate)
...
3. REPLAY LOOP
   for each bar_time in timeline:
       ...
       e. Для каждого ENTER:
          ...
          - Если passed: BacktestExecutor.OpenPosition (fill at next bar open + spread + slippage)
          - Если rejected: записать в gate_stats
       f. Для каждого EXIT:
          - BacktestExecutor.ClosePosition (fill at next bar open)
       g. Для каждого MODIFY_SL:
          - BacktestExecutor.ModifySL
       h. BacktestExecutor.CheckSLTP (проверить SL/TP hit на текущем баре для всех открытых)
...
**Timeline:** Для M30 × 13 символов — все символы имеют одинаковые timestamps (синхронные бары). Timeline = ~17,500 шагов для годового теста. Это ~17,500 TCP roundtrip-ов.
...
**Entry price:** Сигнал на close текущего бара, вход на open следующего бара + per-symbol spread + per-symbol slippage (из cost_model.json). Это совпадает с research (entry = next bar open + costs).

**SL/TP проверка:** На каждом баре проверяем все открытые позиции:
- BUY SL hit: bar.Low ≤ SL → close at SL (или bar.Open если gap)
- BUY TP hit: bar.High ≥ TP → close at TP (или bar.Open если gap)
- Приоритет при одновременном SL+TP hit: для BUY — Open < SL → SL first; Open > TP → TP first; иначе SL (conservative)

**Warmup:** Стратегия mtf_pullback просит 1000 M30 баров warmup. Загружаем 1000 баров ДО начала теста. Первый TICK содержит все warmup бары, ENTER подавляется. Со второго TICK — торговля активна.
...
public class BacktestExecutor
{
    ...
    // Cost model (resolved to execution points per symbol)
    Dictionary<string, ResolvedCost> SymbolCosts;

    // Методы
    BtPosition? OpenPosition(StrategyAction action, Bar nextBar,
                              string symbol, LotResult lot, string strategy, int magic);
    // ↑ spread/slippage берутся из SymbolCosts[symbol] автоматически
    ...
    // R-result: pnl_points / initial_risk_points
...
{
    public double SpreadPoints;          // spread в execution points
    public double SlippagePoints;        // slippage в execution points
    public string AssetClass;
    public double OriginalSpread;        // исходное значение из JSON (для display)
    public string Unit;                  // "pips", "points", "usd"
}
...
public class BtPosition
{
    ...
    public double SpreadCost, SlippageCost;  // per-trade cost tracking
    public int Magic;
...
public class BtTrade
{
    ...
    public double Pnl, RMultiple;
    public double SpreadCost, SlippageCost, Commission;  // cost breakdown
...
Переиспользуем **существующий** RiskManager без модификаций. Создаём изолированный StateManager (in-memory или temp SQLite) с бектест-профилем и позициями.

### Какие гейты работают

| Gate | Статус | Комментарий |
|------|--------|-------------|
| G1 Mode | ✅ Адаптирован | mode="backtest" → пропускает (как virtual) |
...
│  │ │ Neg months: 20/12  │  Max DD days: 102                 │  │
│  │ └───────────────────────────────────────────────────┘    │  │
│  │ ┌─ Summary ($-mode) ───────────────────────────────┐    │  │
...
Research метрики сохраняются в `backtest_runs` для повторных сравнений.
...
5. **DashboardServer.cs** — команды `bt_get_data_coverage`, `bt_download_bars`, `bt_download_progress`
6. **index.html** — секция Data в Tester tab (progress bars, download button, cost display)

**Тест:** загрузить год M30 для 13 символов, проверить в SQLite. Убедиться что cost model парсится и конвертируется в execution points.

### Этап 2: Replay Engine (2-3 сессии)

1. **BacktestExecutor.cs** — виртуальное исполнение (open/close/SL/TP/P&L) с per-symbol costs
2. **BacktestEngine.cs** — основной replay loop:
   - Загрузка баров из BarsHistoryDb
   - Построение timeline
   - Запуск StrategyProcess
   - Побарный прогон: TICK → ACTIONS → Execute (с cost model)
   - SL/TP проверка на каждом баре
...
4. **DashboardServer.cs** — команды `bt_run`, `bt_progress`, `bt_cancel`
...
1. **BacktestEngine.cs** — расчёт метрик:
   ...
   - $-based: PF, WR, Sharpe, MaxDD($), Calmar
   - Gate stats, cost breakdown (spread/slippage/commission totals)
2. **DashboardServer.cs** — команда `bt_result` (полный результат)
3. **index.html**:
...
2. Сохранение результатов прогонов (для сравнения "до/после" и research ref)
...
Margin1Lot зависит от текущей цены и leverage. В бектесте цена меняется. Для точности можно пересчитывать margin_1lot на каждом баре, но это ovekill для валидации.

**Решение:** Запросить margin_1lot один раз при старте теста. Погрешность ~3-5% за год — приемлемо для валидации переноса.

### 10.3 Cost Model — per-symbol spread и slippage

MT5 bars не содержат исторический spread. Глобальный spread/slippage неточен — GBPNZD имеет spread 3.0 pips, а EURUSD — 1.1 pips.

**Решение:** Файл `cost_model.json` с per-symbol данными:

```json
{
  "units": { "forex": "pips", "indices": "points", "metals_crypto": "usd" },
  "symbols": {
    "EURUSD": { "asset_class": "forex", "spread": 1.1, "slippage": 0.6 },
    "GBPNZD": { "asset_class": "forex", "spread": 3.0, "slippage": 1.8 },
    "XAUUSD": { "asset_class": "metal", "spread": 0.6, "slippage": 0.3 },
    "SP500":  { "asset_class": "index", "spread": 0.7, "slippage": 1.0 }
  }
}
```

> `spread_open_widened` хранится в JSON для справки, но **не используется** тестером — алго не торгует на открытии сессии брокера.

**Конвертация единиц в BacktestExecutor:**
- forex (pips): `cost_points = pips × 10` (для 5-digit broker, Point = 0.00001)
- indices (points): `cost_points = points / Point` (SP500 Point = 0.01, spread 0.7 pts = 70 points)
- metals/crypto/energy (USD): `cost_points = usd / Point` (XAUUSD Point = 0.01, spread $0.60 = 60 points)
...
**Загрузка:** `DaemonConfig.CostModelPath` → при старте daemon/backtest парсим JSON → `Dictionary<string, CostEntry>`.

**Расширяемость:** Когда появятся данные по конкретным брокерам — добавляем файлы `cost_model_the5ers.json`, `cost_model_audacity.json` и selector в UI. Пока один файл = default.
...
- Отдельный StateManager (temp SQLite или in-memory)
...
public class BacktestResult
{
    ...
    // ── $-based метрики (для понимания реальных сумм) ──
    ...
    // Cost model impact
    public double TotalSpreadCost;   // суммарный cost spread ($)
    public double TotalSlippageCost; // суммарный cost slippage ($)
    public double TotalCommission;   // суммарная комиссия ($)
...
R-result для каждой сделки считается через существующий `RCalc.GetRResult()`:
- Из `SignalData` извлекаем initial_risk (entry - SL distance)
- R = actual_pnl_points / initial_risk_points
- SL hit → exactly -1.0R (или близко, с учётом gap/slippage)
- TP hit → зависит от RR ratio в стратегии
- Signal exit → variable R (может быть дробным)
- Protector (trailing SL) → обычно положительный R (locked profit)
...
- Исторические спреды (tick-level) — per-symbol cost model достаточен
...
## 13. Файловая структура cost_model.json

```
daemon/
├── config.json
├── cost_model.json          ← per-symbol costs (default)
├── state.db
├── bars_history.db          ← исторические бары (создаётся при первом download)
├── news_calendar.json
└── strategies/
    ├── mtf_pullback/
    └── momentum_cont/
...
Формат файла (40 символов, 5 asset classes):
...
Конвертация в execution points (InstrumentCard.Point):
- **forex pips** → `pips × 10 × Point` (5-digit: 1 pip = 10 points)
- **index points** → `points / Point`
- **metals/energy/crypto USD** → `usd / Point`

BacktestExecutor хранит `Dictionary<string, ResolvedCost>` с уже пересчитанными значениями в Point для каждого символа.
