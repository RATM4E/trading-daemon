...
> ⚠️ DAX — `DAX40`, не `DE40` / `GER40` (DE40 — вариант, не canonical). Japan — `JPN225`, не `JP225`. `pos["symbol"]` в positions всегда содержит canonical имя — Reconciler нормализует брокерские символы через `ToCanonical` при сохранении в БД.
...
| `DE40` / `GER40` вместо `DAX40`, `JP225` вместо `JPN225` в sym конфига | SymbolResolver не найдёт символ при подписке на бары; использовать canonical имена из таблицы выше |
