...
|   |   |-- RiskManager.cs      ~775L   [Phase 10]  13-gate chain (G0-G12),
|   |   |                                              Gate 0: Global Pause (blocks all new entries,
|   |   |                                                timed auto-expire, persisted in daemon_state),
|   |   |                                              Gate 1: "virtual" => Pass(),
...
|   |   |-- AlertService.cs     ~1150L  [Phase 10]  + Telegram keyboard UI (11 buttons, no slash menu),
|   |   |                                              /pause /resume /settings /heartbeat commands,
|   |   |                                              RiskManager integration (SetRiskManager),
|   |   |                                              conversational input (pending command state),
|   |   |                                              MapKeyboardText (emoji→command), rawHtml flag,
|   |   |                                              pause state in /status report
...
|   |
|   |-- Dashboard/                       [Phase 7]
|   |   +-- DashboardServer.cs ~3328L   [Phase 10]  + toggle_pause/get_pause_state commands,
|   |                                                   globalPaused/pauseUntil/pauseReason in terminals response,
|   |                                                   event:pause_state broadcast, SetRiskManager,
|   |                                                   strategy enable/disable, terminal management,
...
|   |
|   |-- wwwroot/                         [Phase 10]
|   |   |-- index.html            ~100L  [Phase 10] Shell only: CSS, CDN scripts, script imports
|   |   +-- js/
|   |       |-- common.js         ~500L  [Phase 10] React hooks (var), ErrorBoundary, WsManager,
|   |       |                                        Badge, ProgressBar, StatusDot, TypeBadge, ModeBadge, Stat
|   |       |-- app.js            ~310L  [Phase 10] useWs hook, TABS, App component, pause UI,
|   |       |                                        header (PAUSE/RESUME + banner + countdown),
|   |       |                                        Emergency/Shutdown/Discovery/TradeChart modals
|   |       +-- tabs/
|   |           |-- terminals.js         [Phase 10] Terminal tiles, ConfigPanel, DiscoveryPanel
|   |           |-- terminal-detail.js   [Phase 10] Equity chart, positions, closed trades
|   |           |-- positions.js         [Phase 10] Positions table with P/L
|   |           |-- strategies.js        [Phase 10] Strategy tiles, enable/disable
...
|       |                                              daily_r, class_leverage, daemon_state (pause persistence),
...
### Phase 10: Dashboard Modularization + Global Pause + Telegram Overhaul (25.02.2026)

Крупный рефакторинг frontend, новая функция Global Pause, полная переработка Telegram бота.

**Dashboard Modularization** -- монолитный `index.html` (1925 строк) разбит на 12 файлов. Shell `index.html` (~100L) содержит только CSS, CDN импорты и `<script type="text/babel" src="...">` теги. `js/common.js` (~500L) -- React hooks (объявлены через `var` для глобальной видимости в Babel standalone), ErrorBoundary, WsManager, утилитарные компоненты. `js/app.js` (~310L) -- useWs hook, роутинг вкладок, App component с модалками. 8 файлов в `js/tabs/` -- по одному на каждую вкладку. Порядок загрузки: common.js → tabs/* → app.js.

**Logarithmic Progress Bars** -- DD прогресс-бары теперь используют логарифмическую шкалу. Малые значения (0.1-1%) визуально различимы, ранее были невидимы на линейной шкале.

**UTC + Local Time** -- хедер показывает три значения: UTC HH:MM:SS (zinc-500), локальное время (zinc-600), дату MM/DD/YYYY (zinc-700).

**Global Pause** -- мягкая пауза торговли без остановки стратегий:
- **Gate 0** в RiskManager -- новый первый гейт перед всеми 12 существующими. Volatile `_globalPaused` с optional timed auto-expire. Блокирует все ENTER, но SL/TP management, виртуальная торговля и мониторинг продолжают работать.
- **StateManager** -- таблица `daemon_state` (key-value store). `LoadPauseState()` / `SavePauseState()` -- пауза переживает рестарт.
- **DashboardServer** -- WS команды `toggle_pause` / `get_pause_state`. `globalPaused/pauseUntil/pauseReason` в ответе `get_terminals`. Broadcast `event:pause_state` всем клиентам.
- **Frontend** -- кнопка ⏸️ PAUSE / ▶️ RESUME в хедере (между Refresh и EMERGENCY). Amber banner под хедером с countdown и reason. Модалка с выбором длительности (∞, 30m, 1h, 2h, 4h) и полем причины.
- **Telegram** -- `/pause [min] [reason]` и `/resume` команды. Pause state в `/status` и `/settings` отчётах.

**Telegram Overhaul** -- полная переработка интерфейса бота:
- **Keyboard UI** -- 11 кнопок в 4 ряда (Status, Positions, P/L, News, Settings, Heartbeat, Pause, Resume, Mute, Unmute, Help). `deleteMyCommands` убирает slash-меню.
- **Button mapping** -- `IsKnownButtonText()` + `MapKeyboardText()` через `Contains()` (не regex -- emoji variation selectors ломали паттерны).
- **Новые команды**: `/settings` (pause/mute/heartbeat status + terminal modes), `/heartbeat` с conversational input (pending command state -- бот спрашивает число, ждёт ответ).
- **rawHtml flag** в `SendAsync` -- предотвращает двойное экранирование HTML в Telegram алертах.
- **Conversational input** -- `_pendingCommand` field. При `/heartbeat` без аргумента бот переходит в режим ожидания числа. Любая кнопка отменяет ожидание.
...
| `StateManager.cs` | ~1737 | P10 | SQLite: профили, позиции, P/L, events, 3SL, active_strategies, symbol_sizing, **strategy_registry**, DeleteTerminalData, **virtual_balance/margin CRUD, virtual_equity, sl_history, trade_snapshots**, **class_leverage**, **GetEffectiveLeverage**, **daemon_state (pause persistence)** | 34/34 |
| `Reconciler.cs` | ~294 | P9.V | Cold/hot reconciliation DB <-> MT5, timezone-aware, **virtual filter** | live-тест |
| `RiskManager.cs` | ~775 | P10 | **13-gate chain** (G0-G12), **Gate 0: Global Pause (timed auto-expire, persistent)**, Gate 1: virtual=>Pass(), Gate 5: **effective leverage + CalcProfit reduce**, Gate 6: **Margin1Lot + effective leverage**, Gate 7: **minImpact**, Gate 10: NoTradeOn toggle, Gate 11: Same Symbol Per Strategy, Gate 12: R-cap | 69/69+ |
...
| `AlertService.cs` | ~1150 | P10 | Telegram Bot API, **keyboard UI (11 buttons)**, heartbeat, commands (/pause /resume /settings /heartbeat), **disconnect alerts**, **conversational input**, MapKeyboardText, rawHtml, AUDIT logging | 9/9 |
...
| `DashboardServer.cs` | ~3328 | P10 | HTTP + WS, **~45 команд**, **toggle_pause/get_pause_state**, **pause state in terminals + broadcast**, strategy enable/disable, terminal management, Sizing tab (multi-format init), leverage detect, **CHECK_SYMBOLS**, **Virtual Trading UI**, news always-on + minImpact filter, toggle_no_trade, **trade chart (digits/drag/bars/PnL)**, **WMI discovery**, **probe_terminal**, **open_strategy_folder**, **virtual PnL cache**, **leverage persistence**, **effective leverage for virtual margin** | live-тест |
| `wwwroot/` (modular) | ~2400 | P10 | **Modularized**: index.html shell + js/common.js + js/app.js + 8× tabs/*.js. **Pause UI** (button/banner/modal), **logarithmic progress bars**, **UTC+local time**, all Phase 9 features preserved | live-тест |
...
Phase 9: Боевое тестирование                 -- done (Virtual Trading + Dashboard Polish +
                                                Schema Unification + R-cap + Optimization + Margin Fix)

Phase 10: Dashboard Modularization + Global Pause + Telegram Overhaul -- done
   [done] Dashboard modularization              -- 1925L monolith → 12 modular files (~2400L total)
   [done] Logarithmic progress bars             -- log scale for DD bars
   [done] UTC + local time display              -- header: UTC + local + date
   [done] Telegram keyboard UI                  -- 11 buttons, no slash menu, emoji→command mapping
   [done] Global Pause (Gate 0)                 -- 13-gate chain, timed auto-expire, persistent
   [done] Pause dashboard UI                    -- button/banner/modal with countdown
   [done] Telegram /pause /resume /settings     -- RiskManager integration
   [done] Telegram /heartbeat                   -- conversational input with pending state
   ...
   --- Phase 10: Dashboard Modularization + Global Pause + Telegram Overhaul (done, 25.02.2026) ---
   [done] Dashboard modularization              -- monolithic index.html (1925L) split into 12 files:
                                                   index.html (shell ~100L), js/common.js (~500L),
                                                   js/app.js (~310L), 8× js/tabs/*.js.
                                                   Babel standalone with var globals, load order via src=
   [done] Logarithmic progress bars             -- DD bars now use log scale (small values visible)
   [done] UTC + local time display              -- header shows UTC HH:MM:SS + local time + date
   [done] Telegram persistent keyboard          -- 11 button grid (Status/Positions/P∕L/News/Settings/
                                                   Heartbeat/Pause/Resume/Mute/Unmute/Help),
                                                   deleteMyCommands (no slash menu), resize_keyboard
   [done] Telegram keyboard button mapping      -- emoji prefix stripping via Contains() (not regex),
                                                   IsKnownButtonText + MapKeyboardText
   [done] Global Pause (Gate 0)                 -- RiskManager: new Gate 0 before all 12 gates,
                                                   volatile _globalPaused + timed auto-expire,
                                                   StateManager: daemon_state table (key-value),
                                                   LoadPauseState/SavePauseState (survives restart),
                                                   DashboardServer: toggle_pause/get_pause_state WS commands,
                                                   globalPaused in terminals response, event:pause_state broadcast
   [done] Pause dashboard UI                    -- PAUSE/RESUME button in header (amber/animated),
                                                   amber banner with countdown + reason,
                                                   modal: duration selector (∞/30m/1h/2h/4h) + reason input
   [done] Telegram /pause /resume               -- HandlePause (parse [min] [reason]), HandleResume,
                                                   SetRiskManager wiring, rawHtml flag for HTML alerts
...
**Telegram** стал основным каналом мониторинга -- сигналы, блокировки, crash recovery, disconnect alerts, heartbeat, **11 кнопок keyboard UI**, **commands (/pause /resume /settings /heartbeat)**, **conversational input**, **VIRTUAL alerts**. Dashboard дополняет, но Telegram доступен всегда с телефона.
...
- Алерты (AlertService -> Telegram: сигналы, disconnect alerts, heartbeat, **keyboard UI (11 кнопок)**, **commands (/pause /resume /settings /heartbeat)**, **conversational input**, **VIRTUAL prefix**)
...
- Web-панель управления (DashboardServer, ~45 команд, terminal management, **Global Pause toggle/broadcast**, **Virtual Trading UI**, init/reset sizing, R-cap settings, **trade chart with digits/drag/bars**, **async non-blocking polling**, **batch profile loading**)
