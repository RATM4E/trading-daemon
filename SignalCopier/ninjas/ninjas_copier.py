...
SKIP_RISK_ORDER = os.environ.get("SKIP_RISK_ORDER", "false").lower() == "true"
...
def parse_signal(text: str) -> Optional[Signal]:
    ...
    if entry_mkt == sl:
        return None
    # Фильтр мусора: SL дальше 50% от entry — невалидный сигнал
    if abs(entry_mkt - sl) / entry_mkt > 0.5:
        return None
...
class NinjasOKX:
    ...
    def _ts(self) -> str:
        return str(time.time_ns() % 10**14)  # наносекунды, коллизия невозможна
    ...
    async def on_signal(self, signal: Signal):
        ...
        elif SKIP_RISK_ORDER and signal.risk_note:
            log.info(f"SKIP [{sym}]: RISK ORDER (SKIP_RISK_ORDER=true)")
            return
    ...
    async def _check_position(self, sym: str, ps: PositionState):
        ...
        # Если SL или TP ещё не поставлен — попробуем ещё раз
        if not ps.sl_algo_id or (ps.mkt_filled and not ps.tp_order_id and ps.signal.tp1):
            await self._place_sl_tp(sym)
            return
