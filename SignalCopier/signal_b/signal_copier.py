...
TP_DRIFT         = float(os.environ.get("TP_DRIFT", "0.0005"))   # смещение TP внутрь рынка
POLL_INTERVAL    = int(os.environ.get("POLL_INTERVAL", "15"))           # секунд между проверками
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler("signal_copier.log", encoding="utf-8", mode="a"),
    ]
)
...
def load_state() -> dict:
    ...
    try:
        ...
        for sym, d in data.items():
            s = d['signal']
            sig = Signal(
                symbol=s['symbol'], tf=s.get('tf', 'M15'), direction=s['direction'],
...
def parse_signal(text: str) -> Optional[Signal]:
    try:
        ...
        e1  = re.search(r"ENTRY\s*1\s*[:\s]?\s*([\d.]+)", text)
        e2  = re.search(r"ENTRY\s*2\s*[:\s]?\s*([\d.]+)", text)
...
def parse_entry2_update(text: str) -> Optional[Entry2Update]:
    ...
    try:
        ...
        e2_m  = re.search(r"OPEN\s+ENTRY\s*2\s*[:\s]?\s*([\d.]+)", text, re.IGNORECASE)
        tp1   = re.search(r"TP1\s*[:\s]\s*([\d.]+)", text)
        tp2   = re.search(r"TP2\s*[:\s]\s*([\d.]+)", text)
        tp3   = re.search(r"TP3\s*[:\s]\s*([\d.]+)", text)
...
class BaseExecutor(ABC):
    ...
    async def _poll_position(self, symbol: str):
        ...
        while symbol in self.positions:
            ...
            # --- Позиция без ордеров — placement упал до получения ID ---
            if not pos.entry_order_ids and not pos.sl_order_id:
                log.warning(f"[{symbol}] Нет ордеров и нет SL — удаляем из state")
                await self.on_closed(symbol, "NO_ORDERS")
                return
            ...
            if not entry1_filled and pos.entry_order_ids:
                ...
                if status == "closed":
                    ...
                    log.warning(f"[{symbol}] ENTRY1 статус unknown — ждём следующего тика")
            ...
            # --- После заполнения ENTRY1: следим за позицией ---
                ...
                # Проверяем реальный размер позиции на бирже — универсально для OKX и Binance
                try:
                    pos_size = await self._fetch_position_size(sym, pos.signal.direction)
                except Exception as e:
                    log.warning(f"[{symbol}] Опрос позиции: {e}")
                    continue

                if pos_size == 0:
                    # Позиция закрыта — определяем причину по статусу SL ордера
                    sl_status = "unknown"
                    try:
                        sl_status = await self._fetch_order_status(sym, pos.sl_order_id)
                    except Exception:
                        pass

                    if sl_status == "closed":
                        log.info(f"[{symbol}] SL исполнен")
                        ...
                        log.info(f"[{symbol}] Позиция закрыта (TP или вручную)")
                        for oid in pos.entry_order_ids[1:]:
                            try:
                                await self._cancel_order(sym, oid)
                            except Exception:
...
class OKXExecutor(BaseExecutor):
    ...
    async def _sync_state(self):
        """Синхронизация state с биржей при старте — удаляет позиции которых нет на бирже."""
        state = load_state()
        if not state:
            return
        log.info(f"Синхронизация state: {len(state)} позиций...")
        stale = []
        for sym, pos in state.items():
            sig = pos.signal
            okx_sym = self.exchange_symbol(sig)
            try:
                size = await self._fetch_position_size(okx_sym, sig.direction)
            except Exception as e:
                log.warning(f"[{sym}] sync: не удалось проверить биржу: {e} — оставляем")
                continue
            if size == 0:
                log.warning(f"[{sym}] sync: позиции нет на бирже — удаляем из state")
                stale.append(sym)
            else:
                log.info(f"[{sym}] sync: позиция подтверждена ({size} контрактов)")
        for sym in stale:
            pos = state[sym]
            okx_sym = self.exchange_symbol(pos.signal)
            # Отменяем висящие ордера
            for oid in pos.entry_order_ids + pos.tp_order_ids:
                try:
                    await self._cancel_order(okx_sym, oid)
                except Exception:
                    pass
            if pos.sl_order_id:
                try:
                    await self._cancel_order(okx_sym, pos.sl_order_id)
                except Exception:
                    pass
            del state[sym]
        if stale:
            save_state(state)
            log.info(f"Sync: удалено {len(stale)} потеряшек: {stale}")
        self.positions = state
        # Запускаем поллинг для оставшихся позиций
        for sym in self.positions:
            asyncio.create_task(self._poll_position(sym))
        log.info(f"Sync завершена: {len(self.positions)} активных позиций")

        # Обратная проверка — позиции на бирже которых нет в state
        try:
            all_positions = await self.ex.fetch_positions()
            known_symbols = {pos.signal.symbol for pos in self.positions.values()}
            adopted = []
            for p in all_positions:
                contracts = float(p.get("contracts") or 0)
                if contracts == 0:
                    continue
                raw_sym = p.get("symbol", "")       # BTC/USDT:USDT
                base    = raw_sym.split("/")[0]      # BTC
                sym     = base + "USDT"              # BTCUSDT
                if sym in known_symbols:
                    continue
                side      = p.get("side", "long")
                direction = "LONG" if side == "long" else "SHORT"
                entry_px  = float(p.get("entryPrice") or p.get("info", {}).get("avgPx") or 0)
                inst_id   = raw_sym.replace("/", "-").replace(":USDT", "-SWAP")

                # Ищем SL algo ордер
                sl_price  = 0.0
                sl_id     = None
                try:
                    r = await self.ex.private_get_trade_orders_algo_pending({
                        "instId": inst_id, "ordType": "conditional"
                    })
                    pos_side = "long" if direction == "LONG" else "short"
                    for algo in (r.get("data") or []):
                        if algo.get("posSide") == pos_side:
                            sl_price = float(algo.get("slTriggerPx") or algo.get("triggerPx") or 0)
                            sl_id    = algo.get("algoId")
                            break
                except Exception:
                    pass
                ...
                if entry_px == 0:
                    log.warning(f"⚠ ORPHAN: {sym} {direction} — не удалось получить entry price, пропускаем")
                    continue

                # Строим минимальный сигнал
                sig = Signal(
                    symbol=sym, tf="M15", direction=direction,
                    entry1=entry_px, entry2=None,
                    sl=sl_price or entry_px,
                    tp1=tp_price or entry_px,
                    tp2=None, tp3=None,
                    leverage=20, raw="[adopted]",
                )
                ps = PositionState(
                    signal=sig,
                    total_lot=contracts,
                    entry_order_ids=["adopted"],
                    sl_order_id=sl_id,
                    tp_order_ids=[tp_id] if tp_id else [],
                    entry2_placed=False,
                )
                self.positions[sym] = ps
                state[sym] = ps
                adopted.append(sym)
                log.warning(f"⚠ ADOPTED: {sym} {direction} {contracts} контр.  "
                            f"entry={entry_px}  sl={sl_price or '?'}  tp1={tp_price or '?'}")

            if adopted:
                save_state(self.positions)
                log.info(f"Sync: принято {len(adopted)} orphan позиций: {adopted}")
                for sym in adopted:
                    asyncio.create_task(self._poll_position(sym))
        except Exception as e:
            log.warning(f"Обратная проверка не удалась: {e}")
...
class BinanceExecutor(BaseExecutor):
    ...
    def __init__(self):
        ...
        params = {"apiKey": BINANCE_API_KEY, "secret": BINANCE_SECRET,
                  ...
                  "options": {"defaultType": "future",
                               "fetchMarkets": ["linear"],   # пропускаем margin/spot
                               "fetchCurrencies": False}}    # не грузим /sapi/margin
        if BINANCE_BASE_URL:
        ...
        # ThreadedResolver — обходит проблему aiodns на Windows
    ...
    async def _refresh_symbols(self):
        ...
        self._symbols = {i["id"] for i in self._markets.values()
                         if i.get("type") == "swap" and i.get("quote") == "USDT"
                         and i.get("active") and i.get("linear")}
        Path(self.CACHE_FILE).write_text(
            json.dumps({"updated_at": time.time(), "symbols": list(self._symbols)}))
    ...
    async def _max_leverage(self, sym: str) -> int:
        ...
        info = next((v for v in self._markets.values()
                     if v.get("id") == sym and v.get("type") == "swap" and v.get("linear")), {})
        try:
            return int(info.get("limits", {}).get("leverage", {}).get("max", MAX_LEVERAGE) or MAX_LEVERAGE)
        except Exception:
            return MAX_LEVERAGE
    ...
    async def _calc_lot(self, signal: Signal) -> float:
        if not self._markets:
            self._markets = await self.ex.load_markets()
        info = next((v for v in self._markets.values()
                     if v.get("id") == signal.binance_symbol
                     and v.get("type") == "swap" and v.get("linear")), None)
    ...
    async def _place_limit(self, sym: str, side: str, qty: float, price: float,
        ...
        # В hedge mode reduceOnly не нужен — positionSide определяет закрытие
        o = await self.ex.create_order(sym, "LIMIT", side, qty, price, params=params)
        return str(o["id"])
    ...
    async def _fetch_order_status(self, sym: str, order_id: str) -> str:
        try:
            ...
            pass
        # Fallback: STOP_MARKET ордера на Binance не видны через fetch_order —
        # ищем в открытых ордерах
        try:
            open_orders = await self.ex.fetch_open_orders(sym)
            ids = {str(o["id"]) for o in open_orders}
            if str(order_id) in ids:
                return "open"
            # Не в открытых — проверяем через fetch_orders (история)
            orders = await self.ex.fetch_orders(sym, limit=10)
